...
public actor IliadboxClient {
  ...
  public static let appVersion = IliadBarBuildInfo.version

  let baseURL: URL
  private var appToken: String?
  var sessionToken: String?
  let urlSession: URLSession
  /// Retained explicitly because optimized builds may otherwise release the
  /// trust delegate before URLSession finishes all authentication challenges.
  private let trustDelegate: BoxTrustDelegate?

  public private(set) var permissions: [String: Bool] = [:]

  public func hasPermission(_ name: String) -> Bool {
    permissions[name] == true
  }

  public func permissionSnapshot() -> [String: Bool] { permissions }

  public init(config: IbxConfig, urlSession: URLSession? = nil) {
    ...
    if let urlSession {
      ...
      let delegate = BoxTrustDelegate(expectedHost: self.baseURL.host)
  ...
  public func requestAuthorization(deviceName: String) async throws -> AuthorizeResult {
    ...
    let result: AuthorizeResult = try await call(
      "login/authorize/", method: "POST", jsonBody: body, authenticated: false)
  ...
  public func waitForApproval(trackId: Int, onPoll: (@Sendable (String) -> Void)? = nil)
    async throws
  {
    while true {
      let track: TrackResult = try await call(
        "login/authorize/\(trackId)", method: "GET", authenticated: false)
      onPoll?(track.status)
      switch track.status {
      case "granted":
        return
  ...
  public func openSession() async throws -> SessionResult {
    ...
    let session: SessionResult = try await call(
      "login/session/", method: "POST", jsonBody: body, authenticated: false)
  ...
  func authedCall<T: Decodable>(
    ...
    do {
      return try await call(
        path, method: method, jsonBody: jsonBody, formBody: formBody, rawBody: rawBody,
        contentType: contentType, authenticated: true)
  ...
  private func call<T: Decodable>(
    ...
    if let jsonBody {
      ...
      request.setValue(
        contentType ?? "application/octet-stream", forHTTPHeaderField: "Content-Type")
    ...
    guard let envelope = try? JSONDecoder().decode(FbxResponse<T>.self, from: data) else {
      throw (200...299).contains(http.statusCode)
        ? FbxError.invalidResponse : FbxError.http(status: http.statusCode)
    }
